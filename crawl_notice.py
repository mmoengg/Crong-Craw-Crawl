...
# 2. 양천구 가족센터 행사 리스트 크롤링 함수 (최대 2페이지까지 예시)
def crawl_yangcheon_events(max_page=2):
...
# 2. 금천구 가족센터 행사 리스트 크롤링 함수 (최대 2페이지까지 예시)
def crawl_geumcheonfamily_events(max_page=2):
    base_url = "https://geumchfc.familynet.or.kr/center/lay1/program/S295T322C451/recruitReceipt/list.do"
    headers = {'User-Agent': 'Mozilla/5.0'}
    rows = 5
...
df_geumcheon_events = crawl_geumcheonfamily_events()

# 양천구 데이터에는 '번호', '작성자', '파일', '등록일', '조회수' 칼럼 없으니 빈 칼럼 추가
for col in ['번호', '작성자', '파일', '등록일', '조회수']:
    df_yangcheon[col] = '-'
    df_geumcheon_events[col] = '-'
...
df_geumcheon_events = df_geumcheon_events.drop(columns=['번호', '파일', '등록일', '조회수'])
...
df_geumcheon_events = df_geumcheon_events[columns_order]


# 두 데이터 합치기
df_all = pd.concat([df_geumcheon, df_yangcheon, df_geumcheon_events], ignore_index=True)
